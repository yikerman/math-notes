...
\begin{definition}{Partial derivative}{partial-deriv}
...
Other perspective: $\mathbf{r}(t)=
  \begin{pmatrix}
    t \\f(t)
  \end{pmatrix}
$, \;
tangent vector $\mathbf{r}'=
  \begin{pmatrix}
    1 \\f'(t)
  \end{pmatrix}
$.
...
\begin{definition}{Tangent plane}{tangent-plane}
...
\[
  \mathbf{u} = \pd{\mathbf{r}}{x}=
  \begin{pmatrix}
    1 \\0\\f_{x}
  \end{pmatrix}
  ,
  \qquad
  \mathbf{v} = \pd{\mathbf{r}}{y}=
  \begin{pmatrix}
    0 \\1\\f_{y}
  \end{pmatrix}
\]
...
\begin{definition}{Linear approximation}{linear-approx}
...
\begin{theorem}{}{chain-rule-one-var}
...
\begin{theorem}{Chain rule — two intermediate variables}{chain-rule-two-var}
...
\begin{definition}{Directional derivative}{dir-deriv}
  \[
...
\begin{definition}{2D Gradient}{gradient-2d}
...
\begin{example}
  ...
  \[
    ...
    =
    \begin{pmatrix}
      3x^{2}\cos(xy)-x^{3}y\sin(xy) \\-x^{4}\sin(xy)
    \end{pmatrix}
  \]
...
\begin{definition}{Tangent plane to a level surface}{tangent-plane-level}
...
\begin{example}[Gradient ascent]
  ...
  \[
    ...
    = \lambda
    \begin{pmatrix}
      f_{x}(\mathbf{r}(t)) \\f_{y}(\mathbf{r}(t))
    \end{pmatrix}
    \;\Longleftrightarrow\;
...
\begin{definition}{Local maximum}{local-max}
...
\begin{definition}{Critical point}{critical-pt}
...
\begin{definition}{Saddle point}{saddle-pt}
...
\begin{theorem}{Second derivative test}{second-deriv-test}
