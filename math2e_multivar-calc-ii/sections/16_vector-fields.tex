...
\begin{remark}
Note that $\mathbf{r}(t) = \begin{bmatrix}x(t)\\y(t)\\z(t)\end{bmatrix}$.
\end{remark}

By the multivariable chain rule,
\[
    \frac{d}{dt}f(\mathbf{r}(t))
    = \pd{f}{x}\frac{dx}{dt} + \pd{f}{y}\frac{dy}{dt} + \cdots
    = \begin{bmatrix} \dfrac{\partial f}{\partial x} \\[8pt] \dfrac{\partial f}{\partial y} \\[8pt] \vdots \end{bmatrix}
      \cdot
      \begin{bmatrix} \dfrac{dx}{dt} \\[8pt] \dfrac{dy}{dt} \\[8pt] \vdots \end{bmatrix}
...
\[
    ...
    = \int_a^b \frac{d}{dt}f(\mathbf{r}(t))\dd{t}
    = f(\mathbf{r}(b)) - f(\mathbf{r}(a))
\]
\end{proof}

As a result, on a conservative vector field $\mathbf{F} = \nabla f$, the line integral $\int_C \mathbf{F}\cdot d\mathbf{r}$ is
...
For vector field $\mathbf{F}(x, y)=\begin{bmatrix}P(x, y)\\Q(x, y)\end{bmatrix}$ with continuous first-order partials,

\[
    \mathbf{F} \text{ is conservative} \implies \pd{P}{y} = \pd{Q}{x}.
\]

And the converse only holds if the domain is simply connected.

\begin{remark}
Informally ``simply connected'' means the domain is one piece with no holes — every loop can be shrunk to a point without leaving the domain.
\end{remark}
...
\[
    \mathbf{F} = \begin{bmatrix}P \\ Q\end{bmatrix} = \begin{bmatrix} f_x \\ f_y \end{bmatrix}
\]
...
$\mathbf{F} = \begin{bmatrix}P \\ Q\end{bmatrix}$ has continuous partial
derivatives on an open region containing $D$, then
\[
    \oint_C \mathbf{F}\cdot d\mathbf{r}
    = \int_C P\dd{x} + Q\dd{y}
    = \iint_D \left(\pd{Q}{x} - \pd{P}{y}\right)dA
\]
\end{theorem}
