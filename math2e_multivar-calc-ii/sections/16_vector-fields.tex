  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.7, >=latex]
			...
			\foreach \x/\y in {%
					...
					\pgfmathsetmacro{\rcubed}{(\x*\x+\y*\y)*sqrt(\x*\x+\y*\y)}
					\pgfmathsetmacro{\dx}{-\x*0.8/\rcubed}
					\pgfmathsetmacro{\dy}{-\y*0.8/\rcubed}
...
\begin{example}[Sphere]
  Parametrize the sphere $x^2+y^2+z^2 = a^2$. Recall spherical coordinates
  $x = a\sin\varphi\cos\theta$, $y = a\sin\varphi\sin\theta$, $z = a\cos\varphi$, so
  \[
  ...
  Alternatively, parametrize the back half ($x\leq 0$) of $x^2+y^2+z^2 = a^2$ by $(y,z)$:
  \[
    \mathbf{r}(y,z) =
    \begin{bmatrix}
      -\sqrt{a^2-y^2-z^2} \\
      ...
      z
    \end{bmatrix}
    ,\qquad y^2+z^2\leq a^2.
  \]
\end{example}

\subsubsection{Tangent Planes}

\begin{definition}{Smooth Surface and Tangent Plane}{smooth-surface-tangent-plane}
  Let $\mathbf{r}(u,v)\colon\R^2\to\R^3$ be a parametric surface. If
  $\pd{\mathbf{r}}{u}\neq\mathbf{0}$ and $\pd{\mathbf{r}}{v}\neq\mathbf{0}$, the surface is
  \emph{smooth}. The \emph{tangent plane} at $\mathbf{r}(u_0,v_0)$ is
  \[
    \operatorname{span}\left\{\pd{\mathbf{r}}{u},\,\pd{\mathbf{r}}{v}\right\},
  \]
  and the \emph{normal vector} to the plane is
  \[
    \mathbf{n} = \pd{\mathbf{r}}{u}\times\pd{\mathbf{r}}{v}.
  \]
\end{definition}
...
\begin{center}
  \begin{tikzpicture}[scale=1.4, >=latex]
		% dome silhouette
		\draw[thick] (-1.5,0) arc[start angle=180, end angle=0, radius=1.5];
		% equator: solid front, dashed back
		\draw[thick] (-1.5,0) arc[start angle=180, end angle=360, x radius=1.5, y radius=0.4];
		\draw[thick, dashed] (-1.5,0) arc[start angle=180, end angle=0, x radius=1.5, y radius=0.4];
		% tangent point on dome
		\coordinate (P) at (0.55,1.0);
		\fill (P) circle (0.035);
		% tangent frame (in tangent plane) and outward normal
		\draw[->, thick, blue!70] (P) -- ++(0.75,-0.15) node[right]{$\mathbf{r}_u$};
		\draw[->, thick, blue!70] (P) -- ++(-0.15,0.55) node[above left]{$\mathbf{r}_v$};
		\draw[->, very thick, red!70] (P) -- ++(0.45,0.75) node[above right]{$\mathbf{n}$};
	\end{tikzpicture}
\end{center}
...
\begin{example}
  ...
  \textbf{Step 1 — solve for $(u,v)$.} Setting $\mathbf{r}(u,v) = \ve{1,1,3}$ gives
  $u^2=1,\,v^2=1,\,u+2v=3$, so $(u,v) = (1,1)$.

  \textbf{Step 2 — partials and normal.}
  \[
    \pd{\mathbf{r}}{u} =
    \begin{bmatrix}
      2u \\ 0 \\ 1
  ...
  \textbf{Step 3 — plane equation.} Using $(\mathbf{x}-\mathbf{x}_0)\cdot\mathbf{n}=0$,
  \[
    \begin{bmatrix}
      -2 \\ -4 \\ 4
    \end{bmatrix}
...
\begin{definition}{Surface Area}{surface-area}
  Let $\mathbf{r}(u,v)\colon\R^2\to\R^3$ parametrize a surface $S$ over a region
  $D\subseteq\R^2$, with $S$ covered exactly once. The \emph{surface area} of $S$ is
  \[
    \operatorname{Area}(S) = \iint_D \lVert\mathbf{r}_u\times\mathbf{r}_v\rVert\dd{A},
    \qquad
    \mathbf{r}_u = \pd{\mathbf{r}}{u},\;\mathbf{r}_v = \pd{\mathbf{r}}{v}.
  \]
\end{definition}

\begin{example}[Surface area of a sphere]
  Compute the surface area of the sphere of radius $a$ using
  \[
    \mathbf{r}(\varphi,\theta) =
    \begin{bmatrix}
      a\sin\varphi\cos\theta \\
      a\sin\varphi\sin\theta \\
      a\cos\varphi
    \end{bmatrix}
    ,\qquad \varphi\in[0,\pi],\;\theta\in[0,2\pi].
  \]
  ...
  \textbf{Normal.}
  \[
    \mathbf{n} = \pd{\mathbf{r}}{\varphi}\times\pd{\mathbf{r}}{\theta}
    =
    \begin{bmatrix}
  ...
  \begin{align*}
    \lVert\mathbf{n}\rVert
     & = \sqrt{a^4\sin^4\varphi\cos^2\theta + a^4\sin^4\varphi\sin^2\theta + a^4\sin^2\varphi\cos^2\varphi} \\
     & = \sqrt{a^4\sin^4\varphi + a^4\sin^2\varphi\cos^2\varphi}                                            \\
     & = \sqrt{a^4\sin^2\varphi\,(\sin^2\varphi+\cos^2\varphi)}
    = \sqrt{a^4\sin^2\varphi}
    = a^2\sin\varphi
  \end{align*}
  (using $\sin\varphi\geq 0$ on $[0,\pi]$).

  \textbf{Integrate.}
  \begin{align*}
    \operatorname{Area}
     & = \iint_D \lVert\mathbf{n}\rVert\dd{A}
    = \int_0^{\pi}\!\int_0^{2\pi} a^2\sin\varphi\dd{\theta}\dd{\varphi} \\
     & = 2\pi a^2\int_0^{\pi}\sin\varphi\dd{\varphi}
    = 2\pi a^2\cdot 2 = 4\pi a^2.
  \end{align*}
\end{example}

\begin{note}
  For a surface given as a graph $z = f(x,y)$ over $D$, parametrize by $\mathbf{r}(u,v) = \ve{u,\,v,\,f(u,v)}$. Then
  \[
    \mathbf{n} = \pd{\mathbf{r}}{u}\times\pd{\mathbf{r}}{v}
    =
    \begin{bmatrix}
    ...
    \end{bmatrix}
    ,\qquad
    \lVert\mathbf{n}\rVert = \sqrt{1+\lVert\nabla f\rVert^2},
  \]
  so
  \[
    \operatorname{Area} = \iint_D \sqrt{1+\lVert\nabla f\rVert^2}\dd{A}.
  \]
\end{note}

\subsection{Surface Integrals}

Two forms: scalar and vector.

\subsubsection{Surface Integral of a Scalar Function}

\begin{definition}{Surface Integral of a Scalar Function}{surface-integral-scalar}
  Let $S$ be a surface parametrized by $\mathbf{r}(u,v)$ for $(u,v)\in D$, and let
  $f(x,y,z)$ be a scalar function. The \emph{surface integral of $f$ over $S$} is
  \[
    \iint_S f\dd{S}
    = \iint_D f(\mathbf{r}(u,v))\,
    \underbrace{\lVert\mathbf{r}_u\times\mathbf{r}_v\rVert\dd{A}}_{\dd{S}}.
  \]
\end{definition}

\begin{example}
  Evaluate $\displaystyle\iint_S yz\dd{S}$ where $S$ is the surface $z = x + y^2$ for
  $x\in[0,1],\,y\in[0,2]$.

  Parametrize $S$ by $\mathbf{r}(u,v) = \ve{u,\,v,\,u+v^2}$ over $D = [0,1]\times[0,2]$.
  Using the graph formula $\lVert\mathbf{r}_u\times\mathbf{r}_v\rVert = \sqrt{1+\lVert\nabla(u+v^2)\rVert^2}$,
  \[
    \lVert\mathbf{r}_u\times\mathbf{r}_v\rVert = \sqrt{1 + 1 + 4v^2} = \sqrt{2 + 4v^2}.
  \]
  Then
  \begin{align*}
    \iint_S yz\dd{S}
     & = \int_0^1\!\int_0^2 y(x+y^2)\sqrt{2+4y^2}\dd{y}\dd{x}                    \\
     & = \int_0^2 y\sqrt{2+4y^2}\left(\tfrac{1}{2}+y^2\right)\dd{y}              \\
     & = \int_2^{18}\frac{w^{3/2}}{32}\dd{w}                                     \\
     & \qquad\text{(sub.\ $w = 2+4y^2$, $y\dd{y}=\dd{w}/8$, $\tfrac12+y^2=w/4$)} \\
     & = \frac{1}{80}(18^{5/2} - 2^{5/2})
    = \frac{968\sqrt{2}}{80}
    = \frac{121\sqrt{2}}{10}.
  \end{align*}
\end{example}
...
\begin{definition}{Oriented Surface}{oriented-surface}
  A surface $S$ is \emph{orientable} if a unit normal $\mathbf{n}$ can be chosen at each point
  $(x,y,z)\in S$ that varies continuously over $S$. Such a choice is an \emph{orientation};
  the pair $(S,\mathbf{n})$ is an \emph{oriented surface}.
\end{definition}
...
\begin{center}
  \begin{tikzpicture}[scale=1, >=latex]
		\draw[thick] (-1.5,0) .. controls (-1,0.6) and (1,0.6) .. (1.5,0)
		.. controls (1.3,-0.4) and (-1.3,-0.4) .. (-1.5,0);
		\foreach \x in {-1, 0, 1} {
				\draw[->, thick, red!70] (\x,0.35) -- ++(0,0.7);
			}
		\node at (1.4,1.0) {$\mathbf{n}_1$};
		\begin{scope}[xshift=4cm]
			\draw[thick] (-1.5,0) .. controls (-1,0.6) and (1,0.6) .. (1.5,0)
			.. controls (1.3,-0.4) and (-1.3,-0.4) .. (-1.5,0);
			\foreach \x in {-1, 0, 1} {
					\draw[->, thick, red!70] (\x,0.3) -- ++(0,-0.7);
				}
			\node at (1.4,-1.0) {$\mathbf{n}_2$};
		\end{scope}
	\end{tikzpicture}
\end{center}

\textbf{Unit normal formulas.} For a graph $z = g(x,y)$, the upward unit normal is
\[
  \mathbf{n} = \frac{1}{\sqrt{1+\lVert\nabla g\rVert^2}}
  \begin{bmatrix}
    -g_x \\ -g_y \\ 1
  \end{bmatrix}
  .
\]
For a general parametrization $\mathbf{r}(u,v)$,
\[
  \mathbf{n} = \frac{\mathbf{r}_u\times\mathbf{r}_v}{\lVert\mathbf{r}_u\times\mathbf{r}_v\rVert}.
\]

\begin{remark}
  Orientation is determined by the order of the cross product: swapping $u$ and $v$ flips $\mathbf{n}$.
\end{remark}

\subsubsection{Surface Integral of a Vector Field (Flux)}

\begin{definition}{Surface Integral of a Vector Field}{surface-integral-vector}
  Let $\mathbf{F}$ be a continuous vector field on an oriented surface $S$ parametrized by
  $\mathbf{r}(u,v)$ for $(u,v)\in D$, with orientation given by $\mathbf{r}_u\times\mathbf{r}_v$.
  The \emph{surface integral} (or \emph{flux}) of $\mathbf{F}$ across $S$ is
  \[
    \iint_S \mathbf{F}\cdot\dd{\mathbf{S}}
    = \iint_D \mathbf{F}(\mathbf{r}(u,v))\cdot
    \underbrace{(\mathbf{r}_u\times\mathbf{r}_v)\dd{A}}_{\dd{\mathbf{S}}}.
  \]
\end{definition}

\begin{example}[Flux across a sphere]
  Find the flux of $\mathbf{F}(x,y,z) = \ve{z,\,y,\,x}$ across the unit sphere with outward orientation.

  Parametrize: $\mathbf{r}(\varphi,\theta) = \ve{\sin\varphi\cos\theta,\,\sin\varphi\sin\theta,\,\cos\varphi}$,
  $\varphi\in[0,\pi],\,\theta\in[0,2\pi]$. From the earlier sphere computation,
  \[
    \mathbf{r}_\varphi\times\mathbf{r}_\theta =
    \begin{bmatrix}
    ...
    .
  \]
  \textbf{Test outward.} At $(\varphi,\theta) = (\pi/2,0)$, $\mathbf{r}_\varphi\times\mathbf{r}_\theta = \ve{1,0,0}$
  at the point $(1,0,0)$. This points outward, so the orientation is correct.

  Then $\mathbf{F}(\mathbf{r}(\varphi,\theta)) = \ve{\cos\varphi,\,\sin\varphi\sin\theta,\,\sin\varphi\cos\theta}$, and
  \begin{align*}
    \iint_S \mathbf{F}\cdot\dd{\mathbf{S}}
     & = \int_0^{2\pi}\!\int_0^{\pi}
    \begin{bmatrix}
    ...
    \end{bmatrix}
    \dd{\varphi}\dd{\theta}                                                   \\
     & = \int_0^{2\pi}\!\int_0^{\pi}\bigl(2\sin^2\varphi\cos\varphi\cos\theta
    + \sin^3\varphi\sin^2\theta\bigr)\dd{\varphi}\dd{\theta}                  \\
     & = 0 + \underbrace{\int_0^{2\pi}\sin^2\theta\dd{\theta}}_{=\pi}
    \cdot\underbrace{\int_0^{\pi}\sin^3\varphi\dd{\varphi}}_{=4/3}
    = \frac{4\pi}{3}.
  \end{align*}
\end{example}

\begin{example}[Flux across a paraboloid + disk]
  Let $\mathbf{F}(x,y,z) = \ve{0,\,y,\,-z}$ and let $S$ be the closed surface consisting of
  \begin{itemize}
    \item $S_1$: the paraboloid $y = x^2 + z^2$ for $0\leq y\leq 1$,
    \item $S_2$: the disk $x^2 + z^2 \leq 1$ at $y = 1$,
  \end{itemize}
  oriented outward.
  ...
  \begin{center}
    ...
    \begin{tikzpicture}[tdplot_main_coords, scale=0.9, >=latex]
			...
			% paraboloid (a few curves)
			\foreach \t in {0, 30, 60, 90, 120, 150, 180} {
					\draw[thick, blue!60, domain=0:1, smooth, samples=20, variable=\r]
					plot ({\r*cos(\t)}, {\r*\r}, {\r*sin(\t)});
				}
			% rim circle at y=1
			\draw[thick] plot[domain=0:360, samples=40, smooth, variable=\a]
			({cos(\a)}, 1, {sin(\a)});
		\end{tikzpicture}
  \end{center}

  \textbf{$S_1$ paraboloid.} Parametrize $\mathbf{r}(x,z) = \ve{x,\,x^2+z^2,\,z}$, $D = \{x^2+z^2\leq 1\}$.
  \[
    ...
    .
  \]
  At $(x,z) = (0,0)$, this gives $\ve{0,-1,0}$, which points in $-y$ direction --- outward at the
  paraboloid's vertex. Then $\mathbf{F}(\mathbf{r}) = \ve{0,\,x^2+z^2,\,-z}$ and
  \[
    \mathbf{F}\cdot(\mathbf{r}_x\times\mathbf{r}_z) = -(x^2+z^2) - 2z^2 = -x^2 - 3z^2.
  \]
  Switching to polar $(x,z) = (r\cos t,\,r\sin t)$,
  \[
    \iint_{S_1}\mathbf{F}\cdot\dd{\mathbf{S}}
    = -\int_0^{2\pi}\!\int_0^1 (r^2\cos^2 t + 3r^2\sin^2 t)\,r\dd{r}\dd{t}
    = -(\pi + 3\pi)\cdot\frac{1}{4}
    = -\pi.
  \]

  \textbf{$S_2$ disk.} Parametrize $\mathbf{r}(x,z) = \ve{x,\,1,\,z}$, $D = \{x^2+z^2\leq 1\}$.
  \[
    \mathbf{r}_x\times\mathbf{r}_z
    = \det
    \begin{bmatrix}
    ...
    .
  \]
  This points in $-y$ direction, but the outward normal at the disk is $+\hat{\mathbf{j}}$. Negate:
  use $\mathbf{n}\dd{A} = \ve{0,1,0}\dd{A}$. Then $\mathbf{F}(\mathbf{r}) = \ve{0,1,-z}$, so
  $\mathbf{F}\cdot\mathbf{n} = 1$, and
  \[
    \iint_{S_2}\mathbf{F}\cdot\dd{\mathbf{S}} = \iint_D 1\dd{A} = \pi.
  \]

  \textbf{Total.}
  \[
    \iint_S \mathbf{F}\cdot\dd{\mathbf{S}}
    = \iint_{S_1}\mathbf{F}\cdot\dd{\mathbf{S}} + \iint_{S_2}\mathbf{F}\cdot\dd{\mathbf{S}}
    = -\pi + \pi = 0.
  \]
\end{example}
...
Stokes' theorem relates the line integral of a vector field around the boundary
curve $C$ of an oriented surface $S$ to the flux of the curl across $S$.

\begin{center}
  \tdplotsetmaincoords{70}{120}
  \begin{tikzpicture}[tdplot_main_coords, scale=1, >=latex]
		% dome surface S
		\draw[thick] (-1.5,0,0) arc[start angle=180, end angle=360, x radius=1.5, y radius=0.4];
		\draw[thick, dashed] (-1.5,0,0) arc[start angle=180, end angle=0, x radius=1.5, y radius=0.4];
		\draw[thick] (-1.5,0,0) .. controls (-1.2,0,1.3) and (1.2,0,1.3) .. (1.5,0,0);
		% normal vectors on surface
		\foreach \x in {-0.9, 0, 0.9} {
				\pgfmathsetmacro{\zv}{1.3 - 0.55*\x*\x}
				\draw[->, thick, red!70] (\x,0,\zv) -- ++(0,0,0.7) node[above]{$\mathbf{n}$};
			}
		% boundary curve C with orientation arrows
		\foreach \a in {30, 150, 270} {
				\draw[->, thick, blue!70] ({1.5*cos(\a)},{0.4*sin(\a)},0)
				arc[start angle=\a, end angle=\a+20, x radius=1.5, y radius=0.4];
			}
		\node at (-1.9,0,0.8) {$S$};
		\node[below] at (0,0,-0.5) {$C$};
	\end{tikzpicture}
\end{center}

\textbf{Orientation convention (right-hand rule).} The orientations of $S$ and $C$
are \emph{compatible} when, with the right-hand fingers curling along $C$, the thumb
points in the direction of $\mathbf{n}$.
...
\begin{center}
  ...
  \begin{tikzpicture}[tdplot_main_coords, scale=1, >=latex]
		...
		\draw[thick] (-1.5,0,0) .. controls (-1.2,0,1.3) and (1.2,0,1.3) .. (1.5,0,0);
		% inward normal
		\draw[->, very thick, red!70] (0,0,1.3) -- ++(0,0,-0.8) node[right]{$\mathbf{n}$};
		\node at (-2.2,0,0.6) {\small surface orientation: inward};
		% C orientation
		\foreach \a in {30, 150, 270} {
				\draw[<-, thick, blue!70] ({1.5*cos(\a)},{0.4*sin(\a)},0)
				arc[start angle=\a, end angle=\a+20, x radius=1.5, y radius=0.4];
			}
		\node[below] at (1.6,0,-0.4) {\small $C$ orientation};
	\end{tikzpicture}
\end{center}

\begin{theorem}{Stokes' Theorem}{stokes}
  Let $S$ be an oriented, piecewise-smooth surface in $\R^3$ bounded by a simple,
  closed, piecewise-smooth curve $C$ with positive orientation (compatible with
  the orientation of $S$ by the right-hand rule). Let $\mathbf{F}$ be a vector
  field whose components have continuous partial derivatives on an open region
  containing $S$. Then
  \[
    \oint_C \mathbf{F}\cdot\dd{\mathbf{r}}
    = \iint_S \operatorname{curl}\mathbf{F}\cdot\dd{\mathbf{S}}.
  \]
  If $S$ is parametrized by $\mathbf{r}(u,v)$ over $D\subseteq\R^2$, this expands to
  \[
    \iint_S \operatorname{curl}\mathbf{F}\cdot\dd{\mathbf{S}}
    = \iint_D \operatorname{curl}\mathbf{F}(\mathbf{r}(u,v))\cdot(\mathbf{r}_u\times\mathbf{r}_v)\dd{A}.
  \]
\end{theorem}

\begin{remark}
  Green's theorem is the 2D case of Stokes' theorem: for
  $\mathbf{F}=\ve{P,\,Q,\,0}$ on the $xy$-plane with unit normal $\hat{\mathbf{k}}$,
  $\operatorname{curl}\mathbf{F}\cdot\hat{\mathbf{k}} = Q_x - P_y$, so
  $\iint_S\operatorname{curl}\mathbf{F}\cdot\hat{\mathbf{k}}\dd{S} = \iint_D(Q_x-P_y)\dd{A}$.
\end{remark}

\begin{remark}
  The flux of $\operatorname{curl}\mathbf{F}$ depends only on the boundary $C$, not on
  the choice of surface $S$ spanning $C$: any two oriented surfaces with the same
  oriented boundary give the same value of $\iint_S\operatorname{curl}\mathbf{F}\cdot\dd{\mathbf{S}}$.
\end{remark}

\begin{example}
  Evaluate $\displaystyle\oint_C \mathbf{F}\cdot\dd{\mathbf{r}}$ where
  \[
    \mathbf{F}(x,y,z) =
    \begin{bmatrix}
      -y^2 \\ x \\ z^2
    \end{bmatrix}
  \]
  and $C$ is the intersection of the plane $y+z=2$ with the cylinder $x^2+y^2=1$,
  ...
  \begin{center}
    ...
    \begin{tikzpicture}[tdplot_main_coords, scale=0.9, >=latex]
			...
			\draw[thick] plot[domain=0:360, samples=40, smooth, variable=\a]
			({cos(\a)}, {sin(\a)}, 0);
			% ellipse: intersection of y+z=2 with cylinder
			\draw[thick, blue!70] plot[domain=0:360, samples=60, smooth, variable=\a]
			({cos(\a)}, {sin(\a)}, {2-sin(\a)});
			\foreach \a in {45, 165, 285} {
					\draw[->, thick, blue!70]
					({cos(\a)}, {sin(\a)}, {2-sin(\a)})
					-- ({cos(\a+8)}, {sin(\a+8)}, {2-sin(\a+8)});
				}
			\node[right] at (1.1,0,2.2) {$C$};
			\node at (-1.5,0,1.5) {\small $x^2+y^2=1$};
			\node at (1.4,1.4,2.5) {\small $y+z=2$};
		\end{tikzpicture}
  \end{center}

  By Theorem~\ref{thm:stokes}, choose any surface $S$ with $\partial S = C$. The simplest
  is the elliptical disk in the plane $y+z=2$:
  \[
    S\colon\quad z = 2-y,\qquad (x,y)\in D=\{x^2+y^2\leq 1\}.
  \]

  \textbf{Parametrize $S$.} Let $g(x,y)=2-y$ and
  $\mathbf{r}(x,y) = \ve{x,\,y,\,2-y}$. Then
  \[
    ...
    .
  \]
  The $z$-component is positive, matching the upward (counterclockwise-from-above)
  orientation of $C$.
  ...
  \begin{align*}
    \oint_C \mathbf{F}\cdot\dd{\mathbf{r}}
     & = \iint_S \operatorname{curl}\mathbf{F}\cdot\dd{\mathbf{S}}
    = \iint_D \operatorname{curl}\mathbf{F}\cdot(\mathbf{r}_x\times\mathbf{r}_y)\dd{A} \\
     & = \iint_D
    \begin{bmatrix}
      0 \\ 0 \\ 1+2y
    \end{bmatrix}
    ...
    \dd{A}                                                                             \\
     & = \iint_D (1+2y)\dd{A}
    = \int_0^{2\pi}\!\int_0^1 (1+2r\sin\theta)\,r\dd{r}\dd{\theta}                     \\
     & = \int_0^{2\pi}\left[\tfrac{1}{2}+\tfrac{2}{3}\sin\theta\right]\dd{\theta}
    = \pi.
  \end{align*}
\end{example}
